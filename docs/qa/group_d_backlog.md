...
- **AK-13** — Bump Wasmtime past `RUSTSEC-2026-0114`. April 2026 Wasmtime advisory batch flagged via transitive dep. Wasmtime typically patches its advisories promptly. Identify which workspace dep pulls Wasmtime, find the minimum upgrade that clears the advisory, verify no breakage. The `deny.toml` ignore expires `2026-05-30`. If unresolved by expiry, `cargo-deny` re-fails and forces re-prioritization. Not blocking Phase 1.
...
- **AK-15** — Audit-redaction invariant on `SecretsFacade`. Commit 5 deleted `vault.rs`'s `test_secret_never_in_audit`. Confirm or add equivalent coverage on the facade: writing a secret value via `facade.set_secret` must NEVER cause the plaintext value to appear in the audit log JSON. Quick check: grep `kernel/src/secrets/tests.rs` for any test asserting plaintext absence in audit; if none, add one. Verification at HEAD `3561eca8` shows the facade has zero `audit_trail.append_event` calls in `get_secret` / `set_secret` / `delete_secret` — the ADR documents auditing as a Phase 1 deliverable but the wiring is currently a placeholder. AK-15 covers BOTH (a) wiring the audit append calls into the facade dispatch loops AND (b) adding a regression test asserting plaintext absence in the resulting audit JSON. Not a correctness gap in the strict sense — the facade currently writes no audit rows at all, so no leak is possible — but the invariant must have a dedicated test once the audit wiring lands. Logical successor to AK-2 (capability-ledger HITL prompt; same dispatch surface). Depends on: nothing structural.
