...
- **BS-OAUTH-UNMIGRATED** — Residual OAuth-credential surface not covered by Bug AK Commits 1-5 (which handled Twitter OAuth1 + 6 static LLM api_keys + 1 HTTP connector site). Outstanding migrations to `SecretsFacade`: (a) auth/OIDC `client_secret` at `auth/src/config.rs:84-90` (inline literal or `NEXUS_OIDC_SECRET` env), migrate to scope `"auth.oidc"` name `"client_secret"`; (b) Matrix `access_token` at `connectors/messaging/src/matrix.rs:46` (currently `std::env::var("NEXUS_MATRIX_ACCESS_TOKEN")`), migrate to scope `"messaging.matrix"` name `"access_token"`; (c) WhatsApp `access_token` at `connectors/messaging/src/whatsapp.rs:63` (env-only), migrate to scope `"messaging.whatsapp"` name `"access_token"`; (d) WhatsApp `verify_token` at `connectors/messaging/src/whatsapp.rs:65` (webhook verification secret), migrate to scope `"messaging.whatsapp"` name `"verify_token"`. Out of BS scope (greenfield, not migration): Teams `access_token` at `integrations/src/providers/teams.rs:135-153` (caller-supplied at runtime; no persistence today); OIDC authorization-code `access_token` at `auth/src/oidc.rs` (in-memory only; persistence is a new feature). Consume `SecretAuditCtx` from AK-2 at each call seam; use `SecretAuditCtx::startup()` for connector-init reads and inline-construct from caller context where available. Priority: medium. Related: Bug AK parent (line 453), AK-2 (efb06aa2), AK-11 partial-status above. **CLOSED PARTIAL (this commit):** sites (b) Matrix, (c) WhatsApp access_token, (d) WhatsApp verify_token migrated to SecretsFacade via AK-2 Phase 3c pattern (global::try_facade + SecretAuditCtx::startup). Site (a) auth/OIDC re-scoped — original premise (raw-TOML migration from `~/.nexus/config.toml`) infeasible because NexusConfig has no `auth` member and config.toml is envelope-encrypted. Re-scoped work tracked under BX-AUTH-OIDC-MIGRATION below. Note: bullet citations `:63`/`:65` for whatsapp.rs are drifted (actual code at `:62`/`:64` at this commit's HEAD); not corrected since ticket is now closed.

- **BT-AUDIT-EVENT-CORRELATION** — `tests/integration/tests/swarm_harness_scenario_d.rs:179-199` asserts `report.shared_run_ids.is_empty()` with a comment "flips when AK-2 lands". AK-2 (efb06aa2) wired `run_id` into the kernel audit row via `SecretAuditCtx`, but scenario_d's coordinator run never invokes `SecretsFacade` — no audit events are emitted with the run_id, so the intersection stays empty (vacuously). Two implementation paths to make the comment match reality: (a) modify scenario_d to invoke a `SecretsFacade` call during the coordinator run with a ctx whose agent_id matches the run_id; (b) add a coordinator-side audit hook that emits an event keyed by run_id at node-task boundaries (the "future swarm-audit commit" the test comment anticipates). Path (b) is the more principled — establishes coordinator/audit cross-trail correlation independently of credential touches. Priority: low (cosmetic; AK-2's ledger work is complete on its own merits). Surfaced during AK-2 acceptance review (Phase 4 unilateral).

- **BU-PUBLISH-EXECUTOR-CTX** — `PublishExecutor` trait method `credentials_present(&self)` at `agents/social-poster/src/swarm_entry.rs:208` performs a pre-flight credential read but does NOT receive `&AgentExecutionContext`. AK-2 (efb06aa2) used `SecretAuditCtx::startup()` at this call site as a documented compromise — the audit row attributes the read to "startup" rather than to the calling agent. If true caller-capability attribution on pre-flight credential reads becomes required (audit forensics, multi-agent debugging), thread `&AgentExecutionContext` through the `PublishExecutor` trait. Touches: every `PublishExecutor` impl, every adapter that invokes `credentials_present`. Priority: low (defensible compromise; not blocking). Surfaced during AK-2 implementation (Phase 3e unilateral).

- **BV-CI-STRESS-GATE** — `scripts/ci-local.sh` rust-tests-core job runs at default thread count, which matches the architect's interactive-shell scheduling but does NOT match gitlab-runner's cgroup-confined process scheduling. Result: shared-state test races (per BR-INPUT-RACE-REFACTOR, fixed at 43a107b2) can pass ci-local locally and fail on GitLab CI. Add an explicit stress-test invocation to ci-local's rust-tests-core job: after the default-threads run passes, re-run kernel lib tests at `--test-threads=16` (or `2x num_cpus`) to surface interleave-sensitive bugs before push. Cost: ~30-60s additional wall-clock per ci-local invocation. Note: this is a CI hygiene gate, NOT a fix for the underlying anti-pattern (shared-static rate limiter); BR-INPUT-RACE-REFACTOR covers the architectural cleanup. Priority: low (hygiene). Surfaced during BR diagnosis (pipeline #2522852881 failure on 27fb8bd0).

- **BW-OAUTH-DISK-PERSISTENCE** — Plaintext-on-disk OAuth credential persistence in Tauri-layer code not covered by BS-OAUTH-UNMIGRATED (which scopes to connector-init reads). Two surfaces in `app/src-tauri/src/commands/apps.rs`: (M1) `read_oauth_setting` at lines 932-944 reads `~/.nexus/oauth_settings.json` for `{provider_id}_client_secret` keys (gmail, outlook, generic); called from apps.rs:1051, 1054, 1812 with `.or_else` env-then-file fallback semantics. (M2) `read_messaging_token` at lines 914-930 reads `~/.nexus/messaging_tokens/<platform>.json` for bot tokens (Slack/Discord/Telegram type integrations). Both plaintext-on-disk — the Bug AK anti-pattern. Writer counterparts unverified at filing time — preflight required to find or confirm absence (BS lean preflight found no writer in `app/src-tauri/src/` via grep; may live in frontend code calling a Tauri command, or operators may place these files manually). Migration target: SecretsFacade with scope candidates `"oauth.<provider>"` and `"messaging.<platform>"`. M2's multi-platform aspect (Slack/Discord/Telegram/etc.) may compound the work — sub-tickets per platform possible. Surfaced during BS lean preflight. Priority: medium. Related: Bug AK parent (line 453), BS-OAUTH-UNMIGRATED, AK-2 (efb06aa2).

- **BX-AUTH-OIDC-MIGRATION** — Re-scoped from BS-OAUTH-UNMIGRATED Site (a) after halted Phase 0 found the original premise infeasible. AuthConfig (`auth/src/config.rs:8-36`) has two fields `client_secret: Option<String>` and `client_secret_env: Option<String>` that have NO production load-from-disk path today: NexusConfig (`kernel/src/config.rs:10-42`) has no `auth` member, the only production construction site is `enterprise.rs:882` calling `AuthConfig::default()`, and `~/.nexus/config.toml` is envelope-encrypted. The two fields are field-level dead code. **Locked scope (Option A2 + global facade access):** (1) Drop both `client_secret` and `client_secret_env` from AuthConfig (`auth/src/config.rs:13-17`); (2) Rewrite `resolve_client_secret(&self) -> Option<String>` to use `nexus_kernel::secrets::global::try_facade()` internally — no signature change — reads from scope `"auth.oidc"` name `"client_secret"` with `SecretAuditCtx::startup()`; EnvBackend handles `NEXUS_AUTH_OIDC_CLIENT_SECRET` by default naming; (3) Update 1 production caller at `auth/src/oidc.rs:181` (no behavior change — still consumes `Option<String>`); (4) Update 4 test fixtures that set these fields: `auth/src/config.rs:109-110`, `auth/src/config.rs:122-123`, `auth/src/lib.rs:144-147`, `auth/src/oidc.rs:350-356` (test_config); (5) Update 3 tests that exercised inline-vs-env paths: `auth/src/config.rs:114`, `auth/src/config.rs:126`, `auth/src/lib.rs:136` — likely replace with facade-injection tests using kernel's test_helpers (verify pattern from AK-2 test suite). **NO migration helper, NO migration table marker, NO config.toml rewrite** — nothing exists on disk to migrate from. Estimated 6-8 file touches, single commit. Priority: medium. Related: Bug AK parent (line 453), BS-OAUTH-UNMIGRATED (closed PARTIAL), AK-2 (efb06aa2 — SecretAuditCtx pattern source), BW-OAUTH-DISK-PERSISTENCE (separate work — real plaintext-on-disk OAuth surface in `app/src-tauri/src/commands/apps.rs`). **CLOSED (this commit):** AuthConfig fields `client_secret` + `client_secret_env` dropped (`auth/src/config.rs`); `resolve_client_secret(&self) -> Option<String>` rewritten to use `nexus_kernel::secrets::global::try_facade()` at scope `"auth.oidc"` name `"client_secret"` with `SecretAuditCtx::startup()`; rustfmt-compliant braced-closure form per Move A precedent. 3 test fixtures updated (`config.rs` Default impl, `lib.rs:auth_config_serde_roundtrip_preserves_all_fields`, `oidc.rs:test_config`); 2 obsolete tests dropped (`config.rs:resolve_inline_secret`, `config.rs:resolve_env_secret` — inline path no longer exists; env path now routes through facade which is intentionally NOT installed in unit tests per kernel test convention, so resolution coverage moves to `kernel::secrets` test suite). 1 production caller at `oidc.rs:181` requires no change (signature preserved). Operator-facing env var name (via EnvBackend rule `format!("NEXUS_{}_{}", scope.to_uppercase(), name.to_uppercase())`): `NEXUS_AUTH.OIDC_CLIENT_SECRET` (period in scope kept as-is per kernel-wide convention; `std::env::var` accepts it on Linux). `nexus-kernel` added as path dependency to `auth/Cargo.toml`. Files touched: 4 (`auth/Cargo.toml`, `auth/src/config.rs`, `auth/src/lib.rs`, `auth/src/oidc.rs`) plus this backlog entry. All 30 nexus-auth tests pass.
...
- **AK-11** — **PARTIAL** (post-AK-2, efb06aa2). Static-LLM and runtime-read paths satisfied: Bug AK Commit 3 migrated the 6 static `LlmConfig.<provider>_api_key` fields to scope `"llm"`; AK-2 (efb06aa2) wired `SecretAuditCtx` through the 4 nexus-swarm provider api_key reads; `chat_llm.rs::api_key_for_provider` already resolves via the facade with arbitrary string IDs (criterion C3 incidentally met by side-effect of those two commits). Scope taxonomy locked as generic `"llm"` (criterion C2 — "candidate: `llm.providers`" was advisory language; collision risk between dynamic-entry IDs and reserved well-known names like `"anthropic"` is handled by Tauri-side input validation, not vault-namespace surgery). REMAINING SCOPE: (C1) drop the dead `LlmProviderEntry.api_key: String` field in `kernel/src/config.rs`; (C4) update the LlmProviderEntry struct shape post-removal; (C5-write) implement the Tauri write path (`save_provider_api_key` or `save_llm_provider_settings` writing user-supplied keys for dynamic providers into scope `"llm"` with `entry.id` as name); (C6) add the per-entry resolution path in `connectors/llm/src/gateway.rs` so dynamic-entry providers' api_keys are looked up at request time — currently greenfield work, gateway today consumes only the 12 static `<provider>_api_key` fields. Depends on: nothing structural. Original entry: Migrate `NexusConfig.llm.providers: Vec<LlmProviderEntry>` per-entry `api_key` fields to the `SecretsFacade`. Each entry has a user-defined `id` (e.g. `"my-openai-1"`) and its own `api_key: String`. Variable count, dynamic content. Requires a hierarchical scope/name scheme — candidate: vault scope `"llm.providers"`, name = `entry.id` — and runtime hydration on read since IDs are not statically known. Out of Phase 1 scope (Bug AK Commit 3 covers the six static `LlmConfig.<provider>_api_key` fields only). Touches: `kernel/src/config.rs::LlmProviderEntry`, the Tauri provider-list commands that read and write the providers vector, `connectors/llm/src/gateway.rs` per-entry resolution path. Depends on: nothing structural — current `SecretsFacade::set_secret` / `get_secret` already accepts arbitrary `(scope, name)` pairs.
