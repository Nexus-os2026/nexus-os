...
/// `trusted_keys` are the Ed25519 public keys (raw bytes) accepted by this sandbox.
...
pub fn verify_wasm_signature<'a>(
    ...
    for key_bytes in trusted_keys {
        if let Ok(true) = CryptoIdentity::verify(
    ...
    // Check if signature is even parseable (64 bytes is correct length)
...
/// Sign wasm bytes with a CryptoIdentity. Returns wasm || signature (64 bytes).
/// Used by publishers and test helpers.
pub fn sign_wasm_bytes(wasm_bytes: &[u8], identity: &CryptoIdentity) -> Vec<u8> {
    let signature = identity.sign(wasm_bytes).unwrap_or_default();
    let mut signed = Vec::with_capacity(wasm_bytes.len() + signature.len());
...
pub fn test_keypair() -> (CryptoIdentity, Vec<u8>) {
    use sha2::Digest;
    let seed = sha2::Sha256::digest(b"nexus-test-key-deterministic");
    let mut seed_bytes = [0u8; 32];
    seed_bytes.copy_from_slice(&seed);
    let identity = CryptoIdentity::from_bytes(SignatureAlgorithm::Ed25519, &seed_bytes).unwrap();
    let vk = identity.verifying_key().to_vec();
    (identity, vk)
}
...
mod tests {
    ...
    fn wrong_key_rejected() {
        ...
        let other_id =
            CryptoIdentity::from_bytes(SignatureAlgorithm::Ed25519, &other_bytes).unwrap();
        let other_vk = other_id.verifying_key().to_vec();
