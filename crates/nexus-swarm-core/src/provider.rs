//! Provider abstraction shared by all swarm providers.
//!
//! Async, carries privacy classification, and exposes health-checking +
//! capabilities metadata for the Router. Lives in `nexus-swarm-core` so
//! agent crates can call `ctx.provider.invoke(...)` without depending on
//! the orchestration crate.

use crate::profile_classes::{CostClass, PrivacyClass, ReasoningTier};
...
fn cost_class_rate_per_1k_tokens(class: CostClass) -> u64 {
    match class {
        CostClass::Free => 0,
        CostClass::Low => 1,
        CostClass::Standard => 5,
